...
from fast_tmp.amis.schema.amis_enums import ActionTypeEnum
...
from fast_tmp.amis.schema.zs import ZSTable
from .schema.forms import Column, Mapping
from .schema.forms.amis_form_enums import FormWidgetSize, ItemModel
from .schema.forms.widgets import (
    DateItem,
    DatetimeItem,
    NumberItem,
    SelectItem,
...
def get_columns_from_model(
    model: Type[Model],
    include: Tuple[Union[str,Column], ...] = (),
) -> List[Column]:
    """
    ...
    for name in include:
        ...
        if field is None:  # 自定义字段，默认全部返回字符串类型
            res.append(Column(name=name, label=name))
        else:
            if isinstance(field, (IntEnumFieldInstance, CharEnumFieldInstance)):
                res.append(
                    Mapping(
                        name=field,
                        label=field,
                        map={k: v for k, v in field.enum_type.choices.items()},
                    )
                )
                # fixme:处理开关字段
            # elif isinstance(field_type,):#fixme:处理特殊字段，比如json字段，或者图片、开关等需要特殊显示的类型。
                ...
                logger.error("can nout list man")
            else:
                res.append(Column(name=name, label=name))
    return res
