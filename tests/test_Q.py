...
async def run():
    ...
    await event.save()
    event2 = Event(name="Test", tournament_id=tournament.id)
    await event2.save()
    ...
    for team in event.participants:
        print(team.id)
    x1=await Event.filter(participants__name="Team 1")
    x2 = await Event.filter(participants__name="Team 1").annotate(pid="id").filter(~Q(id__in=))
